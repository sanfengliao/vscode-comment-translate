...
import {
  ...
  onConfigChange,
  showHoverStatusBar,
  showTargetLanguageStatusBarItem,
} from './configuration';
...
import {
  ITranslateConfig,
  ITranslateRegistry,
  TranslateExtensionProvider,
} from './translate/translateExtension';
...
export async function activate(context: ExtensionContext) {
  let languageId = 2;
  let grammarExtensions: IGrammarExtensions[] = extensions.all
    .filter(({ packageJSON }) => {
    ...
    .map(({ packageJSON, extensionPath }) => {
      ...
      const languages: ITMLanguageExtensionPoint[] = contributesLanguages.map(
        (item: any) => {
          return {
            id: languageId++,
            name: item.id,
  ...
  if (env.remoteName) {
    ...
    let innergrammarExtensions: IGrammarExtensions[] = inner
      .filter(({ packageJSON }) => {
        return packageJSON.contributes && packageJSON.contributes.grammars;
      })
  ...
  translateManager = new TranslateManager(
    ...
    getConfig<number>('maxTranslationLength', 10000),
  ...
  translateExtensionProvider = new TranslateExtensionProvider(
    translateManager,
    buildInTranslate
  );
...
async function readResources(context: ExtensionContext) {
  ...
  return Promise.all(
    resources.map(async (extension) => {
      return {
        packageJSON: JSON.parse(
          await readFileSync(
            `${context.extensionPath}/resources/${extension}/package.json`,
            'utf-8'
          )
        ),
